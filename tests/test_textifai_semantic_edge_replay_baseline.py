...
class TextifAISemanticEdgeReplayBaselineTests(unittest.TestCase):
    def test_replay_input_fixture_structure_is_valid_and_safe(self):
        ...
        for path in REPLAY_INPUT_ROOT.rglob("*"):
            if not path.is_file():
                continue
            self.assertLess(path.stat().st_size, 100_000, path)
    ...
    def test_provider_free_replay_smoke_writes_only_inside_temp_output(self):
        ...
        with tempfile.TemporaryDirectory(prefix="textifai_semantic_edge_replay_") as temp_root:

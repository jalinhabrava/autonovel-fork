...
function BrandMark() {
  ...
  const FULL_LOGO_SRC = '/static/textifai-logo-full.svg';
...
export function AppShell({ active, setActive, children }: { active: SectionId; setActive: (id: SectionId) => void; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen h-screen bg-neutral-100 text-neutral-900 p-2 md:p-4 overflow-hidden"><div className="mx-auto w-full max-w-none h-full rounded-3xl bg-white shadow-xl overflow-hidden border border-neutral-200"><header className="flex items-center justify-between border-b border-neutral-200 px-5 py-4 bg-neutral-50"><button type="button" onClick={() => setActive('overview')} className="text-left"><BrandMark /><div className="text-xs text-neutral-500">Narrative semantic engine · author-facing VaERL workspace</div></button><div className="flex items-center gap-2 text-xs"><StatusChip>Local project</StatusChip><StatusChip>VaERL ready</StatusChip></div></header><div className="grid grid-cols-12 h-[calc(100%-73px)] min-h-0"><aside className="col-span-12 md:col-span-2 border-r border-neutral-200 bg-neutral-50 p-3 flex min-h-0 flex-col"><SidebarNav active={active} setActive={setActive} /><div className="mt-auto pt-4 relative">{menuOpen ? <div className="absolute bottom-16 left-0 right-0 z-20 rounded-2xl border border-neutral-200 bg-white p-2 shadow-xl text-sm"><button type="button" onClick={() => { setSettingsOpen(true); setMenuOpen(false); }} className="w-full rounded-xl px-3 py-2 text-left hover:bg-neutral-100">Ajustes</button><button type="button" onClick={() => setActive('hub')} className="w-full rounded-xl px-3 py-2 text-left hover:bg-neutral-100">Proyecto / Workspace</button><button type="button" className="w-full rounded-xl px-3 py-2 text-left text-neutral-500" disabled>Dev tools · pendiente</button></div> : null}<button type="button" onClick={() => setMenuOpen((v) => !v)} className="w-full rounded-2xl border border-neutral-200 bg-white p-3 text-left hover:bg-neutral-100"><div className="flex items-center gap-2.5"><div className="shrink-0 flex h-9 w-9 items-center justify-center rounded-full bg-neutral-900 text-white font-semibold">A</div><div className="min-w-0 flex-1 leading-tight"><div className="truncate text-sm font-semibold">Autor local</div><div className="truncate whitespace-nowrap text-xs text-neutral-500">Workspace privado</div></div><ChevronDown size={16} className="shrink-0" /></div></button></div></aside><main className="col-span-12 md:col-span-10 bg-white min-h-0 overflow-y-auto">{children}</main></div></div>{settingsOpen ? <SettingsModal onClose={() => setSettingsOpen(false)} /> : null}</div>;
}

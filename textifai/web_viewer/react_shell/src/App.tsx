...
export function App() {
  ...
  if (active === 'editor') content = <section className={editorFullscreen ? 'fixed inset-0 z-30 bg-white overflow-y-auto' : ''}><TopBar title="Editor" subtitle="Solo capítulos/manuscrito desde chapter manifest canónico." actions={<Button onClick={() => setEditDraft({ title: 'Añadir capítulo', body: 'Draft local pendiente de SP-106. No hay persistencia ni write-back en SP-105D.' })}><Plus size={14} className="inline" /> Añadir capítulo</Button>} /><div className="p-5 grid grid-cols-12 gap-5"><aside className={editorFullscreen && editorChapterRailCollapsed ? 'col-span-12 lg:col-span-1 rounded-3xl border border-neutral-200 bg-neutral-50 p-4' : 'col-span-12 lg:col-span-3 rounded-3xl border border-neutral-200 bg-neutral-50 p-4'}><div className="flex items-center justify-between gap-2"><h2 className="font-semibold">{editorFullscreen && editorChapterRailCollapsed ? 'Cap.' : 'Capítulos'}</h2>{editorFullscreen ? <button type="button" onClick={() => setEditorChapterRailCollapsed(!editorChapterRailCollapsed)} className="rounded-xl border border-neutral-200 bg-white px-2 py-1 text-xs">{editorChapterRailCollapsed ? 'Mostrar' : 'Ocultar'}</button> : null}</div>{editorFullscreen && editorChapterRailCollapsed ? null : <div className="mt-4 space-y-2 text-sm">{chapterNotes.map((note) => <button key={note.path} onClick={() => setEditorNotePath(note.path)} className={`w-full rounded-xl border px-3 py-2 text-left ${editorNotePath === note.path ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white border-neutral-200'}`}>{(note as EditorChapter & { name?: string }).display_title || note.name || note.path}</button>)}</div>}</aside><div className={editorFullscreen && editorChapterRailCollapsed ? 'col-span-12 lg:col-span-8' : 'col-span-12 lg:col-span-6'}><div className="rounded-3xl border border-neutral-200 bg-white p-5 min-h-[620px]"><div className="flex items-center justify-between gap-3"><div><div className="text-xs uppercase text-neutral-500">Editor experimental MDXEditor · capítulo real</div><h2 className="mt-2 font-semibold">{((chapterNotes.find((note) => note.path === editorNotePath) as (EditorChapter & { name?: string }) | undefined)?.display_title) || editorNotePath || 'Selecciona capítulo'}</h2><p className="mt-2 text-xs text-amber-700">Modo experimental. No write-back en esta fase.</p></div><div className="flex items-center gap-2"><Button variant="secondary" onClick={() => setEditorMode(editorMode === 'rich' ? 'source' : 'rich')}>{editorMode === 'rich' ? 'Ver source' : 'Ver rich'}</Button><Button variant="secondary" disabled>Guardar llegará en SP-113B</Button><Button variant="secondary" onClick={() => setEditorFullscreen(!editorFullscreen)}><Maximize2 size={14} className="inline" /> {editorFullscreen ? 'Salir fullscreen' : 'Pantalla completa'}</Button></div></div><div className="mt-4 rounded-2xl border border-neutral-200 bg-neutral-50 p-3">{editorMode === 'source' ? <pre className="whitespace-pre-wrap rounded-2xl bg-white p-4 text-sm leading-7 text-neutral-700 max-h-[620px] overflow-auto">{editorDraftMarkdown}</pre> : <MDXEditor markdown={editorDraftMarkdown || ''} onChange={(next) => setEditorDraftMarkdown(next)} readOnly={true} className="max-h-[620px] overflow-auto rounded-2xl bg-white p-2" plugins={[headingsPlugin(), listsPlugin(), quotePlugin(), linkPlugin(), thematicBreakPlugin(), markdownShortcutPlugin(), toolbarPlugin({ toolbarContents: () => <><UndoRedo /><BoldItalicUnderlineToggles /><BlockTypeSelect /><ListsToggle /><CreateLink /></> })]} />}</div></div></div><aside className="col-span-12 lg:col-span-3 space-y-4"><Metric label="Origen" value={String(editorSource?.source_used || 'chapter_manifest')} note={`Panel derecho retenido también en fullscreen · ${chapterNotes.length} capítulos canónicos cargados`} /><div className="rounded-3xl border border-neutral-200 bg-neutral-50 p-5 text-sm text-neutral-600"><div className="rounded-2xl bg-white border border-neutral-200 p-3"><b>Rewrite selection</b><br />Placeholder. Sin LLM.</div><div className="mt-3 rounded-2xl bg-white border border-neutral-200 p-3"><b>Canon risks</b><br />No write-back en esta fase. No write-back en SP-105D.</div></div></aside></div></section>;

...
function EvidenciaModal({ item, onClose }: { item: EvidenciaModalItem; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 p-4"><div className="w-full max-w-3xl rounded-3xl border border-txf-border bg-txf-surface shadow-txf-floating"><div className="flex items-center justify-between border-b border-txf-border p-5"><div><div className="text-xs uppercase tracking-wide text-txf-subtle">Evidencia</div><h2 className="mt-1 text-xl font-semibold">{item.title}</h2><p className="mt-1 text-sm text-txf-subtle">{item.action}</p></div><button type="button" onClick={onClose} className="rounded-full p-2 hover:bg-txf-surface-soft"><X size={18} /></button></div><div className="grid gap-4 p-5 lg:grid-cols-[1.3fr_0.7fr]"><div className="editor-right-panel-scroll space-y-3 pr-1">{refs.length ? refs.map((ref, index) => <div key={`${ref.chapter_id || ref.pointer || 'evidence'}-${index}`} className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Evidencia {index + 1}</div><div className="mt-2 text-sm text-txf-text">Capítulo: {ref.chapter_label || ref.chapter_id || 'pendiente'}</div>{ref.has_text && ref.excerpt ? <div className="mt-2 rounded-xl bg-txf-surface border border-txf-border p-3 text-sm text-txf-text italic">{'«' + ref.excerpt.slice(0, 240) + '»'}</div> : <div className="mt-2 rounded-xl bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">{evidenceMissingReason(sourceMapChunksCount)}</div>}<details className="mt-2"><summary className="cursor-pointer text-xs text-txf-subtle">Detalles técnicos</summary><div className="mt-2 text-xs text-txf-subtle">Pointer: {ref.pointer_short || ref.pointer || 'sin referencia estructurada'}</div></details></div>) : <div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4 text-sm text-txf-subtle">No hay evidence_refs estructurados todavía para este caso.</div>}</div><aside className="editor-right-panel-scroll space-y-3 pr-1"><div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Tipo</div><div className="mt-2 text-sm text-txf-text">{item.raw.review_type || 'decisión editorial'}</div></div><div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Recomendación</div><div className="mt-2 text-sm text-txf-text">{item.raw.recommendation || 'Necesita decisión explícita del autor.'}</div></div><div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Severidad</div><div className={`mt-2 inline-flex rounded-full border px-3 py-1 text-xs uppercase tracking-wide ${severityClass(item.severity)}`}>{item.severity}</div></div></aside></div><div className="flex justify-end border-t border-txf-border p-5"><Button variant="secondary" onClick={onClose}>Cerrar</Button></div></div></div>;
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 p-4"><div className="w-full max-w-3xl rounded-3xl border border-txf-border bg-txf-surface shadow-txf-floating"><div className="flex items-center justify-between border-b border-txf-border p-5"><div><div className="text-xs uppercase tracking-wide text-txf-subtle">Evidencia</div><h2 className="mt-1 text-xl font-semibold">{item.title}</h2><p className="mt-1 text-sm text-txf-subtle">{item.action}</p></div><button type="button" onClick={onClose} className="rounded-full p-2 hover:bg-txf-surface-soft"><X size={18} /></button></div><div className="grid gap-4 p-5 lg:grid-cols-[1.3fr_0.7fr]"><div className="editor-right-panel-scroll space-y-3 pr-1">{refs.length ? refs.map((ref, index) => <div key={`${ref.chapter_id || ref.pointer || 'evidence'}-${index}`} className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Evidencia {index + 1}</div><div className="mt-2 text-sm text-txf-text">Capítulo: {ref.chapter_label || ref.chapter_id || 'pendiente'}</div>{ref.has_text && ref.excerpt ? <div className="mt-2 rounded-xl bg-txf-surface border border-txf-border p-3 text-sm text-txf-text italic">{'«' + ref.excerpt.slice(0, 240) + '»'}</div> : <div className="mt-2 rounded-xl bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">{ref.reason || evidenceMissingReason(sourceMapChunksCount)}</div>}<details className="mt-2"><summary className="cursor-pointer text-xs text-txf-subtle">Detalles técnicos</summary><div className="mt-2 text-xs text-txf-subtle">Pointer: {ref.pointer_short || ref.pointer || 'sin referencia estructurada'}</div></details></div>) : <div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4 text-sm text-txf-subtle">No hay evidence_refs estructurados todavía para este caso.</div>}</div><aside className="editor-right-panel-scroll space-y-3 pr-1"><div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Tipo</div><div className="mt-2 text-sm text-txf-text">{item.raw.review_type || 'decisión editorial'}</div></div><div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Recomendación</div><div className="mt-2 text-sm text-txf-text">{item.raw.recommendation || 'Necesita decisión explícita del autor.'}</div></div><div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4"><div className="text-xs uppercase tracking-wide text-txf-subtle">Severidad</div><div className={`mt-2 inline-flex rounded-full border px-3 py-1 text-xs uppercase tracking-wide ${severityClass(item.severity)}`}>{item.severity}</div></div><div className="rounded-2xl border border-txf-border bg-txf-surface-muted p-4 text-xs text-txf-subtle">Evidence store: {evidenceStoreUsed ? 'sí' : 'no'} · source_map.chunks: {sourceMapChunksCount}</div></aside></div><div className="flex justify-end border-t border-txf-border p-5"><Button variant="secondary" onClick={onClose}>Cerrar</Button></div></div></div>;
}
function EntityRecordTable({ entities, selectedKey, onSelect }: { entities: CanonEntity[]; selectedKey: string; onSelect: (key: string) => void }) { return <div className="overflow-hidden rounded-3xl border border-txf-border"><div className="grid grid-cols-12 bg-txf-surface-soft px-4 py-3 text-xs uppercase tracking-wide text-txf-subtle"><span className="col-span-5">Entidad</span><span className="col-span-2">Tipo</span><span className="col-span-2">Confianza</span><span className="col-span-3">Estado</span></div>{entities.slice(0, 18).map((entity) => { const key = entity.preferred_slug || entity.canonical_name || ''; const active = key === selectedKey; return <button key={key} onClick={() => onSelect(key)} className={`w-full grid grid-cols-12 px-4 py-3 text-sm border-t border-txf-border items-center text-left ${active ? 'bg-txf-surface-muted' : 'bg-txf-surface hover:bg-txf-surface-soft'}`}><span className="col-span-5 font-medium">{entity.canonical_name || key}</span><span className="col-span-2 text-txf-subtle">{entity.entity_kind || 'entity'}</span><span className="col-span-2 text-txf-subtle">{entity.confidence ?? '—'}</span><span className="col-span-3 text-txf-subtle">{entity.review_state || 'ready'}</span></button>; })}</div>; }
...
function GraphInspector({ node, entityCard, noteContent, noteDetail, onEdit, setEditDraft }: { node?: any; entityCard?: CanonEntity | null; noteContent?: string; noteDetail?: NoteDetail | null; onEdit?: (node: GraphCanvasNode) => void; setEditDraft?: (draft: EditDraft) => void }) {
  if (!node) return <aside className="col-span-12 xl:col-span-3 rounded-3xl border border-txf-border p-5 text-sm text-txf-subtle">Selecciona un nodo.</aside>;
        ...
        {([
            ...
            .map(([key, label]) => (
              <div key={key} className="rounded-3xl border border-txf-border bg-txf-surface p-5 text-left">
                <div className="text-xs uppercase tracking-wide text-txf-text">{label}</div>

...
export function App() {
  ...
  const graphStats = { nodes: (graphPayload?.nodes || []).length, edges: (graphPayload?.edges || []).length };
  ...
  if (active === 'graph') content = <section><TopBar title="Graph" subtitle="Superficie React nativa para nodos VaERL, relaciones, backlinks y review markers." actions={<><Button variant="secondary">Mostrar todo</Button><Button variant="secondary">Contexto local</Button></>} />{selectedProjectId ? <NativeGraphSurface graph={graphPayload} selectedNodeId={selectedGraphNodeId} setSelectedNodeId={setSelectedGraphNodeId} setEditDraft={setEditDraft} /> : <div className="m-5 rounded-3xl border border-neutral-200 p-5 text-sm text-neutral-500">Selecciona un proyecto para abrir Graph.</div>}<div className="mx-5 mb-5 hidden"><LegacyEmbed title="Graph legacy dev fallback" src={selectedProjectId ? legacyUrl('graph', selectedProjectId) : '#'} /></div></section>;

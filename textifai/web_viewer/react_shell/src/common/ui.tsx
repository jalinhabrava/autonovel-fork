...
export function Button({ children, variant = 'primary', disabled = false, onClick, className = '', ...rest }: { children: React.ReactNode; variant?: 'primary' | 'secondary'; disabled?: boolean; onClick?: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button type="button" disabled={disabled} onClick={onClick} className={`rounded-2xl px-4 py-2 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50 ${variant === 'primary' ? 'bg-txf-action text-txf-nav-active-text hover:bg-txf-action-hover' : 'bg-txf-surface text-txf-text border border-txf-border-strong hover:bg-txf-surface-soft'} ${className}`} {...rest}>{children}</button>;
}
